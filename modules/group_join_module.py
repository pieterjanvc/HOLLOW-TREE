...
@module.server
def group_join_server(input: Inputs, output: Outputs, session: Session, user, groups, postgresUser):
    ...
    def addToGroup():
        ...
        code = shared.accessCodeCheck(conn, accessCode=input.accessCode(), codeType=2,
        ...
        if invalid.shape[0] > 0:
            shared.inputNotification(session, "accessCode", 
        ...
        _ = shared.executeQuery(
            ...
            "VALUES(?, ?, ?, ?)",
            (int(code.iloc[0]["gID"]), int(user.get()["uID"]), int(code.iloc[0]["adminLevel"]), dt),
        )
