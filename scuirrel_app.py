...
def server(input, output, session):
    ...
    conn = shared.appDBConn(postgresUser=scuirrel_shared.postgresUser)
    cursor = conn.cursor()
...
application, please use the access code provided by your administrator to create an account. {demoInfo}</p>""")),col_widths=12),
            login_ui("login"),
            ),        
        id = "tab")
            )
            ...
            return ui.TagList(ui.navset_pill(
                # TAB 2 - CHAT
                ui.nav_panel("SCUIRREL",
                    chat_ui("chat"), value="cTab"),               
            ))
